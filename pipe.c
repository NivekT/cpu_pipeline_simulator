...
void pipeline_queue_push(Pipeline_Queue *queue, Pipeline_Register *reg)
{
    if (queue == NULL)
    {
        if (DEBUG) printf("pipeline_queue_push: ERROR null queue\n");
        return;
    }

    if (reg == NULL)
    {
        if (DEBUG) printf("pipeline_queue_push: ERROR null reg\n");
    }
...
void process_BEQ(Pipeline_Register *reg)
{
    ...
    if (*exec_FLAG_Z == 1) { // If Zero flag is set, it will be 1
        taken = 1;
        if (DEBUG) {
            printf("process_BEQ: Zero flag is set, we are branching\n");
        }
    } else {
        if (DEBUG) {
            printf("process_BEQ: Zero flag is not set, we are not branching\n");
        }
    }
...
void process_BNE(Pipeline_Register *reg)
{
    ...
    if (*exec_FLAG_Z == 0) { // If Zero flag is set, it will be 1
        taken = 1;
        if (DEBUG) {
            printf("process_BNE: Zero flag is not set, we are branching\n");
        }
    } else {
        if (DEBUG) {
            printf("process_BNE: Zero flag is set, we are not branching\n");
        }
    }
    bp_update(reg->pc, target, taken, 1, reg);
    reg->noop = 1;
...
void process_BGT(Pipeline_Register *reg)
{
    ...
    if (*exec_FLAG_Z == 0 && *exec_FLAG_N == *exec_FLAG_V)  { 
        taken = 1;
        if (DEBUG) {
            printf("process_BGT: We are branching\n");
        }
    } else {
        if (DEBUG) {
            printf("process_BGT: We are not branching\n");
        }
    }
    if (DEBUG)
    {
        printf("process_BGT: predicted_taken = %d\n", reg->predicted_taken);
    }
    bp_update(reg->pc, target, taken, 1, reg);
    reg->noop = 1;
}
...
void process_BLT(Pipeline_Register *reg)
{
    ...
    if (*exec_FLAG_N != *exec_FLAG_V)  { 
        taken = 1;
        if (DEBUG) {
            printf("process_BLT: We are branching\n");
        }
    } else {
        if (DEBUG) {
            printf("process_BLT: We are not branching\n");
        }
    }
    bp_update(reg->pc, target, taken, 1, reg);
    reg->noop = 1;
...
void process_BGE(Pipeline_Register *reg)
{
    ...
    if (*exec_FLAG_N == *exec_FLAG_V)  { 
        taken = 1;
        if (DEBUG) {
            printf("process_BGE: We are branching\n");
        }
    } else {
        if (DEBUG) {
            printf("process_BGE: We are not branching\n");
        }
    }
    bp_update(reg->pc, target, taken, 1, reg);
    reg->noop = 1;
...
void process_BLE(Pipeline_Register *reg)
{
    ...
    if (*exec_FLAG_Z == 1 || *exec_FLAG_N != *exec_FLAG_V)  { 
        taken = 1;
        if (DEBUG) {
            printf("process_BLE: We are branching\n");
        }
    } else {
        if (DEBUG) {
            printf("process_BLE: We are not branching\n");
        }
    }
    bp_update(reg->pc, target, taken, 1, reg);
    reg->noop = 1;
...
void process_CBZ(Pipeline_Register *reg) {
    ...
    if (reg->data[1] == 0) { // If Zero flag is set, it will be 1
        if (DEBUG) {
            printf("process_CBZ: Register equals to zero, we are branching\n");
        }
        taken = 1;
    } else {
        if (DEBUG) {
            printf("process_CBZ: Register does not equal to zero, we are not branching\n");
        }
    }
    bp_update(reg->pc, target, taken, 1, reg);
    reg->noop = 1;
...
void process_CBNZ(Pipeline_Register *reg) {
    ...
    if (reg->data[1] != 0) { // If Zero flag is not set, it will be 0
        if (DEBUG) {
            printf("process_CBNZ: Register does not equal to zero, we are branching\n");
        }
        taken = 1;
    } else {
        if (DEBUG) {
            printf("process_CBNZ: Register equals to zero, we are not branching\n");
        }
    }
    bp_update(reg->pc, target, taken, 1, reg);
    reg->noop = 1;
...
void data_forwarding(uint64_t des, uint64_t rv) {
    ...
    {
        if (DEBUG)
            printf("data_forwarding: found NULL; not forwarding\n");
        return;
    }
    ...
    if (DEBUG)
    {
        printf("data_forwarding: looking at instruction 0x%x\n", opcode);
    }
    ...
    {
        if (DEBUG)
            printf("data_forwarding: found HLT; not forwarding\n");
        return;
    }
        ...
        (0x69A <= opcode && opcode <= 0x69B)) // LSL/LSR (imm)
    {
        if (DEBUG)
        {
            printf("data_forwarding: checking I format instruction\n");
        }
    ...
    { 
        if (DEBUG)
        {
            printf("data_forwarding: checking D format instruction\n");
        }
        ...
        (0x2A0 <= opcode && opcode <= 0x2A7)) // Bcond (BEQ, BNE, BGE, BLT, BGT, BLE)
    {
        if (DEBUG)
        {
            printf("data_forwarding: checking CB format instruction\n");
        }
        uint64_t Rt = get_fragment_of(instruction,0,4);
        if (Rt == des)
        {
